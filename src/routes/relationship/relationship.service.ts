...
export class RelationshipsService {
	...
	async addFriend(userId: ObjectId, friendUsername: { username : string }) {
		...
		try {
			let friendUserInfo = await this.usersService.getUserFrom({ username: friendUsername.username });
			...
			let userIdB = friendUserInfo._id;
			...
			let userB = await this.relationshipsRepository.findOne({ userIdB: userIdB });
